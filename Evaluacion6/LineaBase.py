...
def crearBasesLb(comunidad, ip, port, name):
	...
	ret = rrdtool.create(agentPath + name  + "CPU.rrd",
			"--start", 'N',
			"--step", '60',
...
# 1 Grafica RAM de interfaz
	...
	carga_CPU = int(consultaSNMP(comunidad , ip , port ,'1.3.6.1.2.1.25.3.3.1.2.768'))
	#elif name=="examen":
	...
	ret2 = rrdtool.graphv(  str(agentPath + name) +"CPU.png",
	...
	rrdtool.dump(str(agentPath + name) + 'HDD.rrd' , str(agentPath + name) + 'HDD.xml' )
...
# 1 Grafica RAM de interfaz
	...
	#if float(ultimo_valor)>float(limite):
	#	sendAlertEmail("Agente "+name+"Sobrepasó umbral HDD Go con :"+str(ultimo_valor), str(agentPath + name) +"HDD.png",str(agentPath + name) + "HDD.rrd")
